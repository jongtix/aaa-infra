...
- 저장소: Redis key (`safe_mode:{service}:{module}[:{대상}]`, 예: `safe_mode:collector:token:isa`, `safe_mode:notifier:telegram`)
- 값: `ON` / `OFF`
...
> 결정 배경: [ADR-013](ADR/ADR-013-token-refresh-lock-strategy.md)

**저장**

- Redis: `cache:kis:token:{계좌별칭}` (TTL = 토큰 만료 시각 기반)
- 계좌별칭: 계좌번호 대신 별칭 사용 (예: `isa`, `gold`, `pension`, `stock`, `dc`). 별칭 ↔ 계좌번호 매핑은 환경 변수(`.env`)에서만 관리
- 토큰 갱신 성공 시 Redis TTL 갱신
- **TTL 계산 규칙**: `TTL = (토큰 만료 시각 - 현재 시각) - 10분(안전 여유)`. TTL이 0 이하인 경우 기본값 1시간 적용 (방어 로직)
...
각 Phase 구현 착수 전에 해당 서비스의 테이블 설계 의도를 이 섹션에 추가한다.
DDL 전문(컬럼 타입, 제약조건 등)은 Flyway 마이그레이션 SQL이 단일 소스이며, 이 섹션은 설계 의도·관계·인덱스 전략을 기술한다.

**타입 규칙**: 모든 시간 컬럼은 `DATETIME` 사용. `TIMESTAMP` 사용 금지 (2038년 문제).

**마이그레이션 전략** ([ADR-016](ADR/ADR-016-flyway-schema-migration.md)):
- aaa-collector가 전체 DB 스키마 마이그레이션 소유 (notifier·trader 테이블 포함)
- 마이그레이션 파일 위치: `aaa-collector/src/main/resources/db/migration/`
- 파일 네이밍: `V{번호}__{서비스접두사}_{설명}.sql` (예: `V1__collector_create_stocks.sql`)
- 실행 시점: collector 기동 시 `flyway` 계정으로 Flyway 자동 실행 → Hibernate `ddl-auto=validate` 이중 검증
- notifier·trader: `spring.flyway.enabled=false`
...
| 분류 | 테이블 (예정) | 담당 서비스 |
|------|--------------|------------|
| 종목 마스터 | `stocks` | collector |
| 종목 등급 | `stock_grades` | collector |
...
> 근거: [ADR-014](ADR/ADR-014-redis-key-naming-convention.md)

- **세그먼트 구분**: 콜론(`:`) — `{namespace}:{key}` 형태
- **복합 단어 내부**: 언더스코어(`_`) — `safe_mode`, `tick_price` 등

신규 키 추가 시 아래 패턴을 따른다:

| 예시 키 | 설명 |
|---------|------|
| `cache:kis:token:{계좌별칭}` | KIS API 토큰 캐시 |
| `stream:tick:domestic` | 실시간 틱 스트림 |
| `stream:signal:domestic` | ML 시그널 스트림 |
| `metric:collect:miss:{날짜}` | 수집 누락 건수 메트릭 |
| `safe_mode:collector:token:{계좌별칭}` | 안전 모드 상태 플래그 |
...
- ACL 기반 인증 (`aclfile /etc/redis/users.acl`). `requirepass`·`rename-command` 사용 안 함 (deprecated)
  ...
  - `appuser`: `+@all -@dangerous` — 애플리케이션 서비스 전용. `KEYS`, `DEBUG`, `FLUSHALL`, `FLUSHDB`, `CONFIG`, `SHUTDOWN`, `INFO` 등 차단. Spring Boot 기본 `RedisHealthIndicator`가 `INFO`를 사용하여 NOPERM 에러가 발생하므로, `PING` 기반 커스텀 HealthIndicator로 대체 (→ [ADR-015](ADR/ADR-015-redis-health-indicator-ping-based.md))
...
**경계표 실측 확정 (2026-09-09, SPEC-ANALYZER-INFER-001 M4, REQ-AIF-080/081/111)** — 종전 합성 데이터 기반 초안표(국내 단기 ±2%/±7%·중기 ±4%/±12%, 해외 단기 ±3%/±8%·중기 ±5%/±15%)는 폐기하고 아래 실측표로 대체한다.

- **입력**: A·B등급 유니버스의 백필 완료 실현 수익률(`label_D20`/`label_D60`, `exclude_reason`이 없는 행만 — LABEL-001 데이터셋 조립 경로 재사용). `data_as_of` = 2026-09-08. 행 수: 국내 D20 263,356 / D60 260,796, 해외 D20 207,721 / D60 205,601
- **목표 등급 비율** (4개 조합 동일 적용, 2026-09 사용자 확정): STRONG_SELL 5% / SELL 20% / HOLD 50% / BUY 20% / STRONG_BUY 5% → 경계 = 누적 5%·25%·75%·95% 분위수
- **산출**: `training/boundaries.py` 분위수 역산(TRAIN-001 REQ-AT-040)을 `python -m analyzer.training.boundaries_cli --cache-dir <path> --data-as-of <date> --feature-code-version <str>`로 1회 실행. 반기 재검토 시 동일 CLI 재실행
- **저장소(값의 SSOT)**: `aaa-analyzer/src/analyzer/inference/grade_boundaries.json` — 추론은 `inference/boundaries_store.py`로 이 파일을 읽기만 하며 기동 시 재계산하지 않는다(REQ-AIF-081). 아래 표는 그 JSON을 소수 둘째 자리(%)로 반올림한 문서용 사본이다

**국내 (가격제한폭 ±30%) — 실측**

| 등급 | 단기 (20영업일) | 중기 (60영업일) |
|------|----------------|----------------|
| STRONG_BUY | ≥ +22.66% | ≥ +44.90% |
| BUY | +7.49% ~ +22.66% | +14.51% ~ +44.90% |
| HOLD | -5.59% ~ +7.49% | -8.71% ~ +14.51% |
| SELL | -15.95% ~ -5.59% | -24.78% ~ -8.71% |
| STRONG_SELL | ≤ -15.95% | ≤ -24.78% |

**해외 (가격제한 없음) — 실측**

| 등급 | 단기 (20영업일) | 중기 (60영업일) |
|------|----------------|----------------|
| STRONG_BUY | ≥ +18.42% | ≥ +37.65% |
| BUY | +6.48% ~ +18.42% | +13.31% ~ +37.65% |
| HOLD | -3.53% ~ +6.48% | -4.77% ~ +13.31% |
| SELL | -13.61% ~ -3.53% | -21.19% ~ -4.77% |
| STRONG_SELL | ≤ -13.61% | ≤ -21.19% |

**초안 대조 결과**: 초안보다 HOLD 폭이 2~3배 넓고 전 구간이 우측(양수)으로 치우친다. 원인은 구조적이다 — (1) 초안은 변동성 기반 대칭 추정이었으나 실측은 HOLD 50% 목표 비율의 25%/75% 분위수라 폭이 넓을 수밖에 없고, (2) 2005~2026 장기 우상향 구간 + **현재** A·B등급 유니버스(생존자 편향)의 실현 수익률 분포는 평균이 양수라 경계 전체가 우측으로 이동한다. 원인이 확인된 구조적 어긋남이므로 채택하되(2026-09-10 사용자 확정), 반기 재검토 항목으로 기록한다: 목표 비율 재조정 여부, 유니버스 생존자 편향 완화(상장폐지·등급 하향 종목 포함 여부).

**마진 δ (`grade_margin_delta`, 잠정값)** — 6.6절 4단계 이원 경계(PROMOTE/DEMOTE)의 히스테리시스 마진. 경계표 확정 **후** 4개 조합 전체의 인접 경계 최소 간격(0.10017 — 해외 D20 SELL/HOLD↔HOLD/BUY 간격)의 10%로 유도해 같은 JSON에 `provisional: true`로 저장한다: **δ = 0.010017 (score 축, ≈1.00%p)**. NOTIFIER-FILTER-001 합동 확정 시 JSON의 이 값만 교체하면 되며 구조 변경은 불요하다(REQ-AIF-111).
...
등급은 **거래 가능성(ADTV 절대 임계값)** 과 **학습 가능성(실보유 일봉 데이터 게이트)** 두 축의 AND 조건으로 분류한다. 두 축 모두 `daily_ohlcv` 테이블에서 계산하며, 외부 순위 API에 의존하지 않는다.

| 등급 | 데이터 게이트 (실보유 일봉) | 일평균 거래대금 (ADTV) |
|------|-----------------------------|------------------------|
| A | 750거래일(약 3년) 이상 | HIGH 임계값 이상 |
| B | 250거래일(약 1년) 이상 | LOW 임계값 이상 (HIGH 미만) |
| C | 위 조건 미충족 (또는 비대표 ETF) | 나머지 |

**ADTV 절대 임계값** (시장별 네이티브 통화, 시장별 독립 판정):

| 시장 | HIGH (A) | LOW (B) |
|------|----------|---------|
| KRX (KOSPI/KOSDAQ) | 500억 원 | 100억 원 |
| US (NYSE/NASDAQ/AMEX) | 20억 달러 ($2B) | 5억 달러 ($500M) |

- 데이터 게이트와 ADTV는 AND 조건. 양쪽 모두 충족해야 해당 등급 부여
- ADTV = 최근 20거래일 `daily_ohlcv.trading_value` 평균. KRX는 KRW, US는 USD 네이티브 단위(원화 미환산)로 시장별 독립 산정
- 데이터 게이트는 `listed_date`가 아닌 **실보유 일봉 수**(`COUNT(daily_ohlcv)`)로 측정 — KIS가 미국 상장일을 제공하지 않아(상장일 NULL 다수) 실보유 일봉이 유일하게 시장 무관 적용 가능하며, ML이 실제 학습하는 데이터량과도 일치. 종목별 가용 이력은 daily_ohlcv 과거 백필([BACKFILL-001/002](#))이 채운다
- 비대표 ETF는 C 유지(아래 중복 ETF 대표 선정 참조). F는 구조적 부적합(아래 F등급 판별 참조)

> **설계 이력 (2026-06-24 재설계)**: 이전 기준("상장 7년/3년 + 시장 전체 거래대금 상위 20%/60% percentile")은 KIS 국내 거래량순위 API(FHPST01710000)가 응답 헤더 `tr_cont` 연속조회를 지원하지 않고 단일 페이지 상위 ~30종목만 반환(2026-06-24 실측)하여 "시장 전체 종목 대상" 백분위 계산이 구조적으로 불가능했다. 그 결과 관심종목 대부분이 percentile 100.0 fallback으로 강등되어 A등급이 2종목만 생성되는 장애가 발생했다. 순위 API 의존을 폐기하고, 이미 매일 수집 중인 `daily_ohlcv` 기반 절대 기준으로 전환했다(절대 유동성이 ML 거래 적합성 판정에 시장 상대 순위보다 더 부합). 상세는 SPEC-COLLECTOR-GRADE-004 참조.

**중복 ETF 대표 선정 알고리즘**:
1. 같은 거래소 + 같은 기초지수 추종 ETF 그룹화
   - 그룹화 키: `(거래소, 기초지수 코드, 배수, 방향, 환헤지 여부)` 5튜플
   - 배수/방향/환헤지가 다르면 별개 상품이므로 같은 그룹으로 묶지 않음 (예: KODEX 200 ≠ KODEX 200 레버리지 ≠ KODEX 200선물인버스2X; KODEX 미국S&P500 ≠ KODEX 미국S&P500(H))
2. **최근 20거래일 일평균 거래대금(ADTV) 내림차순**으로 정렬
3. (이론적 방어 장치) ADTV 동률 시 **상장일 빠른 순** — 실무상 동률은 거의 발생하지 않지만, 발생 시 track record가 긴 ETF가 LP 공급·price discovery 면에서 유리
4. (이론적 방어 장치) 상장일까지 동률 시 **종목코드 ASCII 오름차순** — 최종 결정적 tie-breaker, 재계산 시 동일 결과 보장용
5. 1위만 A/B 등급 부여, 나머지는 C 등급 (데이터 수집만)

**선정 기준 근거**:
- **유동성/슬리피지**: ADTV는 호가 스프레드·체결 슬리피지와 가장 강한 역상관을 가지는 단일 지표. AUM이 크더라도 거래가 거의 없는 ETF는 개인 매매자 입장에서 유동성 트랩이므로 AUM을 1순위로 쓰지 않음
- **Price Discovery**: 거래대금 1위 ETF는 가격 발견이 먼저 일어나는 종목이며, 나머지 ETF들은 LP의 NAV 연동 차익거래로 뒤따라가는 구조. ML 학습 데이터 노이즈가 가장 적고 정보량이 많음
- **수집 가능성**: 거래대금은 일봉 시세 API(`daily_ohlcv.trading_value`)로 매일 확실히 수집 가능. 반면 AUM은 KIS API에서 T+1 지연이 있고, TER은 KIS API 전체에서 제공되지 않음
- **단기 스파이크 방지**: 20거래일 롤링 평균으로 테마 이슈 등 단기 거래대금 스파이크를 흡수하여 대표 교체 빈도를 안정화

**재계산 주기 및 안정성**:
- 주 1회 배치 재계산 (매주 월요일 장 시작 전 KST)
- 신규 상장 유예: 거래이력이 20거래일 미만인 ETF는 대표 후보에서 제외
- 거래정지/관리종목: 재계산 시점 거래정지 상태인 ETF는 자동 제외, 해제 시 다음 사이클에 복귀
- 대표 교체 이력은 별도 테이블(`etf_representative_history`)에 기록하여 analyzer의 모델 재학습 트리거로 활용
...
1. **그리드 생성**: 종목별 가상 가격 p의 목록.
   ...
   - 해외 = 가격제한폭이 없으므로 통계적 범위 **±21.5%**(ANALYZER-INFER-001 M7, 2026-09-12 확정 — NAS 프로덕션 DB 해외 유니버스 76종목·278,064행(2007-08-20~2026-09-10)의 일간 등락률 분포에서 SPLIT 아티팩트(확정 이벤트 18건 ±3일) 제외 p99.9=21.25%를 0.5% 그리드 단위로 올림. 초안 ±15%에서 교체)
   - 간격 0.5% (프로퍼티 외부화)
2. **피처 재계산**: 실제 일봉 시계열 마지막에 "종가 = p"인 가상 일봉을 붙여 **가격 파생 피처만** 재계산한다(동결 피처는 마감 값 그대로). 피처 함수는 학습·추론과 동일 모듈(`features/`, 순수 함수)을 호출 — 학습·추론·스윕 3자 간 피처 스큐를 구조적으로 방지. 가상 일봉의 당일 미확정 값(거래량·고가·저가)은 전일 실제값으로 동결한다(ANALYZER-INFER-001 M7 확정, 2026-09-12 — 초안대로 채택). 대안(가상 종가에 비례해 고가·저가·시가를 함께 스케일링)을 실측 비교한 결과, 비례 스케일링은 KBAR 계열 5개 피처(KMID/KLEN/KUP/KLOW/KSFT — 전부 시가 정규화 비율)를 그리드 전 구간에서 수학적으로 상수화시켜(비례 스케일 하에서 분자·분모가 동일 비율로 스케일되어 항등 — 실측: 6개 표본 종목 전부 KUP/KLOW 표준편차 정확히 0.000000) 그리드 위치를 구분하는 정보를 완전히 잃는다. 현재(동결) 규칙은 그리드의 90~97%가 당일 실제 [저가,고가] 범위를 벗어나(실측: domestic 3종목 93~97%, overseas 3종목 90~95%) 그 구간에서 KUP/KLOW가 음수(실제 캔들에서는 결코 나타나지 않는 값, 그리드 포인트의 절반가량에서 발생)가 되는 근사를 감수하지만, 이는 위 "한계" 절이 이미 명시한 "가격만 변하고 나머지는 전일과 동일"이라는 근사의 필연적 귀결이며 트리 모델은 학습 분포 밖 값에 대해서도 리프 외삽으로 안정적으로 처리한다 — 정보를 보존하는 현재 규칙이 더 낫다.
...
4. **이원 경계 산출(히스테리시스 내장)**: score 축에서 등급 경계에 마진 δ(프로퍼티, 초기값은 NOTIFIER-FILTER-001과 합동 확정 — 착수 시점 잠정값은 6.1절 실측 경계표와 함께 산출·저장, INFER-001 M4)를 적용해 **승급(PROMOTE) 경계 = 경계 + δ·방향**, **강등(DEMOTE) 경계 = 경계 − δ·방향** 두 세트를 만들고, 각각 score(p) 곡선과의 교차 가격을 **보간**으로 산출 → 가격 파티션 2세트. 회귀 score가 연속이라 교차점이 그리드 해상도에 묶이지 않는다
5. **조각 병합 후처리(비단조 대응)**: 트리 모델의 비단조성(모멘텀 피처는 가격↑→score↑, 과열 피처는 반대 + 계단형 출력)으로 폭이 좁은 등급 조각이 생길 수 있다 — 폭 < 임계(프로퍼티, **1% 확정 — ANALYZER-INFER-001 M7 실측, 2026-09-12**)인 조각은 인접 밴드에 병합한다(측정 근거: domestic D60 실제 챔피언 모델 표본 12종목 — 원시 밴드(병합 전 run-length) 중 그리드 1칸 폭 "슬리버"는 임계 1%에서 100%(41/41) 병합되고 2칸 이상 폭의 정상 전환 밴드는 0%(0/80) 병합됨 — 슬리버를 전량 흡수하는 최소 임계값이자 정상 전환 정보를 보존하는 경계. 1.5%부터 정상 밴드 병합 시작(16%). 착수 시점 초안값과 동일하게 확정됨 — 측정이 초안을 검증한 사례. overseas D60은 aaa-infra#163 결함으로 실측 불가). 대안인 monotone constraints(가격 파생 피처 단조 제약 학습)는 ANALYZER-TRAIN-001에서 성능 대조 실험 후 채택 여부 결정 [초기 미적용 — 어느 피처에 어느 방향 제약이 옳은지 자체가 모델링 결정이며 성능 훼손 위험]
...
- 커스텀 `releaseRules`는 기본 규칙을 대체하지 않고 우선 평가되며, 매칭되지 않으면 기본 규칙이 fallback으로 적용
