...
**접근 보안**:
- `requirepass` 설정 필수. 비밀번호는 `.env`로 관리 (3.8절 시크릿 보호 참조)
- 위험 명령어 비활성화: `FLUSHALL`, `FLUSHDB`, `CONFIG`, `DEBUG`, `KEYS` → `rename-command`로 차단
...
**Docker 네트워크**:
- `backend` 네트워크: MySQL, Redis, 전체 애플리케이션 서비스 (collector, analyzer, notifier, trader)
- Watchtower: Docker socket만 접근, `backend` 네트워크 연결 불필요
...
| Redis 8.4 (`maxmemory 128mb`) | ~130MB |
| aaa-collector | ~800MB |
| aaa-analyzer | ~500MB |
| aaa-notifier | ~600MB |
| aaa-trader | ~450MB |
...
| Redis 접근 | 5절 접근 보안 | requirepass, 위험 명령어 비활성화, 호스트 바인딩 금지 |
...
| Docker 네트워크 | 10.3절 Docker 네트워크 | backend 네트워크 분리, DB/Redis 호스트 바인딩 금지 |
