...
NAS RAM이 8GB → 32GB로 업그레이드되었다(관련: aaa-infra#119). 이 ADR의 핵심 근거였던 "버퍼풀 상시
상주 불가" 조건이 해소되었다 — `innodb_buffer_pool_size`를 512M→1G로 상향했고(TECHSPEC 10.3절),
DB 전체 크기가 534MB(2026-07-26 실측)로 1G 버퍼풀이 전체 DB를 여유 있게 상회한다. 다만 이미 추가된
`trade_date` 단독 인덱스들은 버퍼풀 상주 여부와 무관하게 날짜 범위 쿼리 자체를 최적화하는 유효한
설계이므로 제거하지 않는다. 보류 테이블 재평가 기준(`EXPLAIN` 풀 스캔 비용 측정)도 그대로 유효하다.
