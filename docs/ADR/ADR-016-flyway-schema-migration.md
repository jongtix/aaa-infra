...
- `collector`: SELECT, INSERT + UPDATE(stocks, stock_grades, backfill_status만). DELETE/DDL 없음[^1]
...
[^1]: 2026-04-04 — `short_sale_overseas` 테이블이 FINRA Daily와 FINRA Short Interest 두 소스를 UPSERT로 병합함에 따라 화이트리스트에 추가됨. → [ADR-017](ADR-017-short-sale-table-split.md) 참조
